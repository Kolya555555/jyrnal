...
<!-- Реалізація пошуку товару у магазині -->
...
<!-- Виведення головного вмісту сторінки -->
      <h1 align="center" ><? echo $category ?></h1>

<?php /*Створюємо маси для виведення товару по категоріях*/
if($category == "Електроніка") {
$array = array("Смарт Часи", "Бездротові навушники", "Наручні Годинники", "Фітнес-браслети", "Bluetooth-колонки", "Інша електроніка");
$kol= 6;
} else if($category == "Подарункові набори") {
$array = array("Сувеніри", "Пакетик Кольоровий", "Інші подарунки");
$kol= 3;
} else{
$array = array("Брелок", "Дитячі приколи", "Різні Іграшки", "Обручі", "Браслети", "Лизуни", "Мячі", "Настільні ігри");
$kol= 8;
}
?>

<!-- Виведення товару по категоріяї, які були вказаны у масиві -->
<?php for($i=0; $i<$kol; $i++) {  ?>
<h2 align="center" >Категорія: <? echo $array[$i] ?></h2>
<div class="container">
    <div class="product-container">
        <?php
            /*Делаем выборку строк і преобразовывем полученные данные в нормальный массив*/
            $electronics = mysqli_query($connect, "SELECT * FROM `electronics` WHERE `electronics`.`category` = '$array[$i]'and `electronics`.`count` > 0");
